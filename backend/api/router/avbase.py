...
async def search(name: str, page: int, isValid: str = Depends(tokenInterceptor)):
    ...
    return JSONResponse(content={"movies": movies, "page": page})


@router.get("/actress/information")
async def search(name: str, isValid: str = Depends(tokenInterceptor)):
    ...
    if info_div:
        ...
        if table:
            ...
            if len(ths) != len(tds):
                return JSONResponse(content={"detail": "表格数据异常，字段数量不匹配"})
...
async def search(url: str, isValid: str = Depends(tokenInterceptor)):
    response = requests.get(url)
    ...
    return JSONResponse(content=movie_info)


@router.get("/keywords")
async def search(keywords: str, page: int, isValid: str = Depends(tokenInterceptor)):
